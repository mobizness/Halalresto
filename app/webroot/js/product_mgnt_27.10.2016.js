...
function optionValidate () {
	...
	if (ProductStoreId == '') {
        ...
        $("#productError").html("Please Select the restaurant");
        ...
        $("[href=#contact]").trigger('click');
        $("#productError").html("Please select the category");
        $("#categoryId").focus();
        return false;
    }
	...
	if (status != 'Failed' && status != 'Pending' && status != '') {
		...
		$.post(rp+'/admin/orders/orderStatus',{'orderId':orderId, 'status':status}, function(response) {
			...
			if (status != 'Delivered') {
				...
				message += (type == 'Delivery') ? 'dispatch system' : 'pickup management';
			}
...
function changeOrderStatus(orderId) {
	...
	if (reason != '') {
		...
		$.post(rp+'/admin/orders/orderStatus',{'orderId':orderId, 'status':'Failed', 'reason':reason}, function(response) {
			$('.ui-loadercont').hide();
			$('#orderList_'+orderId).remove();
			$('#orderMessage').html('Successfully order status changed');
			$('#orderMessage').show();
			setTimeout(function(){
				$('#orderMessage').fadeOut();
			},3000);
	...
	$('#subAddonsList').append(
		'<div class="form-group" id="removeSubaddon_'+subRow+'">'+
			'<div class="col-md-6 col-lg-3 col-md-offset-3">'+
				'<input type="text" class="form-control" name="data[Mainaddon][Subaddon]['+subRow+'][subaddons_name]" placeholder="Subaddon Name" >'+
			'</div>'+
            '<div class="col-md-6 col-lg-2">'+
                '<input type="text" class="form-control" name="data[Mainaddon][Subaddon]['+subRow+'][subaddons_price]" placeholder="Price">'+
            '</div>'+
			'<div class="col-md-6 col-lg-2">'+
	...
	$('#mainsubAddons_'+mainAddonId).append(
		'<div class="form-group" id="removeMainSub_'+mainSubRow+'">'+
			'<div class="col-md-6 col-lg-3 col-md-offset-2">'+
				'<input type="text" class="form-control" name="data[Mainaddon]['+mainAddonId+'][Subaddon]['+mainSubRow+'][subaddons_name]" placeholder="Subaddon Name">'+
			'</div>'+
            '<div class="col-md-6 col-lg-2">'+
                '<input type="text" class="form-control" name="data[Mainaddon]['+mainAddonId+'][Subaddon]['+mainSubRow+'][subaddons_price]" placeholder="Price" >'+
            '</div>'+
			'<div class="col-md-6 col-lg-2">'+
...
function addonsValidate() {
    ...
    if (storeId == '') {
        ...
        $('#addonError').html('Please select category');
        ...
        $('#addonError').html('Please enter valid count');
        $('#MainaddonMainaddonsCount').focus();
        return false;
...
function voucherAddEdit() {
	...
	if (VoucherStoreId == '') {
        ...
        $("[href=#contact]").trigger('click');
        $("#voucherError").html("Please enter voucher code");
	...
	if (!freeDelivery) {
		if (VoucherOfferValue == '') {
	        $("[href=#contact]").trigger('click');
	        $("#voucherError").html("Please enter offer value");
	        $("#VoucherOfferValue").focus();
	        return false;
	    } else if (isNaN(VoucherOfferValue) || VoucherOfferValue <= 0) {
            $("[href=#contact]").trigger('click');
            $("#voucherError").html("Please enter valid offer value");
            $("#VoucherOfferValue").focus();
            return false;
        }
	}
	...
	if (VoucherFromDate == '') {
        $("[href=#contact]").trigger('click');
        $("#voucherError").html("Please select voucher from date");
        $("#VoucherFromDate").focus();
        return false;
    } else if (VoucherToDate == '') {
        $("[href=#contact]").trigger('click');
        $("#voucherError").html("Please select voucher to date");
        $("#VoucherToDate").focus();
        return false;
    }
}
