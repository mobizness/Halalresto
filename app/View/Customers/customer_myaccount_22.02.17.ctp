							...
							<?php if(!empty($order_detail)) { 
		                    	foreach($order_detail as $key => $value){  ?>
			                    	<tr>
										...
										<td>
											<span id="status<?php echo $value['Order']['id']; ?>" class="padding-b-5 col-sm-12"><?php 
												echo ($value['Order']['status'] == 'Collected') ?
													__('Picked up') :  __($value['Order']['status']);?></span></td>
										<td><?php
										...
										<td>
											<span class="ratings<?php echo $value['Order']['id']; ?>"> <?php

												if ($value['Order']['status'] == 'Collected') { ?>
													<a data-target="#trackid" data-toggle="modal" onclick="return viewTrack(<?php echo $value['Order']['id']; ?>);" href="javascript:void(0);">Track Order</a> <?php
												...
												if(!empty($value['Review']['rating'])){
													...
													if($value['Order']['status'] == 'Delivered'){?>
														<a href="javascript:void(0);" onclick = "orderid(<?php echo$value['Order']['id'];?>)"data-toggle="modal" data-target="#reviewPopup"><?php 
														echo __('Review', true); ?></a><?php 
													}
												} ?> 
											</span>
										</td>
