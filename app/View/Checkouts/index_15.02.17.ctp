...
<section class="contact_list">
	<div class="container">
		<div class="chklink_top">
			<div class="col-sm-6">
				<span class="process1 bold"><a href="#"><?php echo __('Your information');?></a></span>
				<span>></span>
					...
					<div class="check_form">
					    ...
					    echo $this->Form->input('paymentMethod',array('type' => 'hidden','name' => 'data[Order][stripepayment_id]','id' => 'stripepayment_id','value' => ''));?>&nbsp;
							...
							<div id="newCard" style="display: none;">
								...
								<div class="col-sm-3">
									<div class="form-group">
										<label class="control-label cl_font" for="month">&nbsp</label><?php
											$curyear    = date("Y");
											$endyear    = $curyear+20;
											$years  = array();
						    ...
						    <div class="col-sm-3 col-xs-12">
							    <div class="form-group">
							    	<label class="control-label cl_font" for="konto"><?php echo __('Account');?> (kr)</label>
...
function checkoutpagintaion(page1,page2){
	...
	if(page1 == '#deliverAddress') {
		...
		if(CustomerFirstName == '') {
			$('#locationError').html('Please enter first name');
			$('#CustomerFirstName').focus();
			return false
		} else if(CustomerLastName == '') {
			$('#locationError').html('Please enter last name');
			$('#CustomerLastName').focus();
			return false
		} else if(CustomerCustomerPhone == '') {
			$('#locationError').html('Please enter phone number');
			$('#CustomerCustomerPhone').focus();
			return false
		} else if(CustomerCustomerEmail == '') {
			$('#locationError').html('Please enter email');
			$('#CustomerCustomerEmail').focus();
			return false
		} else {
			$.post(rp+'checkouts/deliveryLocation',{'id':address, 'orderTypes':orderType}, function(response) {
				if (response == 'fail') {
					(address != '') ? $('#locationError').html('This restaurant wont deliver to your adddress please choose another one') : $('#locationError').html('Please select address');
					$('#selectaddress').focus();
					...
					if (delivery && deliveryTime == "") {     
			           	$('#locationError').html('Restaurant is closed');
			           	$('#getDateTime').focus();
			   			return false;           	
			       	} else {
			       		$(page1).hide();
	    ...
	    $.post(rp + 'checkouts/voucherCheck', {'voucherCode': voucherCode, 
	        ...
	        if (response[0] != 'Failed') {
	            ...
	            if (voucherResponse[3] == 'freeDelivery') {
		            ...
		            $('#voucherPrice_'+storeId).html('kr'+format(voucherResponse[2]));
	            }
	            ...
	            $('.storeSubTotal_'+storeId).html('kr'+format(nowStoreTotal));
