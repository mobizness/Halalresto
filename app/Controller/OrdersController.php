...
class OrdersController extends AppController {
    ...
    public $uses = array('Order', 'ShoppingCart', 'CustomerAddressBook', 'StripeCustomer',
        ...
        'Driver', 'Deal', 'StoreDeliveryArea');
    public $components = array('Stripe', 'Googlemap', 'AndroidResponse', 'Twilio',
        'Notifications', 'PushNotifications', 'Paypal');
    ...
    public function admin_index() {
        ...
        $order_lists = $this->Order->find('all', array(
            'conditions' => array('Order.status' => 'Accepted',
                'Order.order_type' => 'Collection'),
            'order' => array('Order.id DESC')));

        $this->set(compact('order_list', 'status', 'order_lists'));
    }
    ...
    public function admin_reportOrderView($id = null) {
        if (!empty($id)) {
            ...
            $payments = array('Card', 'Wallet', 'paypal');
    ...
    public function conformOrder() {
        ...
        if ($order['order_type'] != 'Collection') {
            ...
            if (empty($freeDeliveryCharge)) {
                //$cityId = $customerDetails['CustomerAddressBook']['google_address'];
                $customerDeliveryAddress = !empty($this->Session->read('customerDeliveryAddress')) ? $this->Session->read('customerDeliveryAddress') : "#$@";
                $DeliveryLocation = $this->DeliveryLocation->query("SELECT * FROM delivery_locations as dl WHERE "
                        . "dl.store_id = $storeId AND '$customerDeliveryAddress' LIKE CONCAT('%',city_id,'%') AND status = 1 LIMIT 1");

                $deliveryCharge = (!empty($DeliveryLocation[0]['dl']['delivery_charge'])) ?
                        $DeliveryLocation[0]['dl']['delivery_charge'] : '0.00';
            }
        }
        ...
        if ($this->request->data['Order']['paymentMethod'] == 'cod') {
            ...
            if ($this->request->data['Order']['paymentMethod'] == 'wallet') {
                ...
                if ($customerDetails['Customer']['wallet_amount'] >= $amount) {
                    ...
                    $this->Session->setFlash(__('Insufficient balance in wallet and payment failed', true), 'default', array('class' => 'alert alert-danger'));
                ...
                if ($response == 'FAILURE') {
                    $this->Session->setFlash(__('Le paiement a échoué', true), 'default', array('class' => 'alert alert-danger'));
    ...
    // delivery cities per store
    public function store_displayDelivery() {
        $this->layout = 'assets';
        $deliveryAreas = $this->DeliveryLocation->find('all', array(
            'conditions' => array(
                'DeliveryLocation.store_id' => $this->Auth->User('Store.id')
            )
                )
        );
        $this->set(compact('deliveryAreas'));
    }

    // delivery cities per store
    public function store_deliveryAdd() {
        ...
        if ($this->request->is('post')) {
            $this->DeliveryLocation->set($this->request->data);
            if (!empty($this->request->data['DeliveryLocation']['store_id']) && ($this->request->data['DeliveryLocation']['store_id'] == $store_id)) {
                $delivery_exists = $this->DeliveryLocation->find('all', array(
                    'conditions' => array('DeliveryLocation.store_id' => $store_id,
                        'DeliveryLocation.city_id' => $this->request->data['DeliveryLocation']['city_id']
                    )
                        )
                );
                if (!empty($delivery_exists)) {
                    $this->Session->setFlash('<p>' . __('Les informations de livraison existent déjà', true) . '</p>', 'default', array('class' => 'alert alert-danger'));
                } else {
                    $this->DeliveryLocation->save($this->request->data, null, null);
                    $this->Session->setFlash('<p>' . __("L'infotmation de livraison a été enregistrée", true) . '</p>', 'default', array('class' => 'alert alert-success'));
                    $this->redirect(array('controller' => 'Orders', 'action' => 'displayDelivery'));
                }
            } else {
    ...
    public function admin_deliveryAdd() {


        $this->layout = 'assets';

        $this->Store->virtualFields = array(
            'name' => "CONCAT(Store.store_name,' (',Store.id,')')"
        );
        $stores = $this->Store->find('list', array(
            'conditions' => array('Store.status' => 1),
        ...
        if ($this->request->is('post')) {
            $this->DeliveryLocation->set($this->request->data);
            if (!empty($this->request->data['DeliveryLocation']['store_id']) && !empty($this->request->data['DeliveryLocation']['city_id']) && !empty($this->request->data['DeliveryLocation']['estimate_delivery_time']) && !empty($this->request->data['DeliveryLocation']['minimum_order']) && !empty($this->request->data['DeliveryLocation']['delivery_charge'])) {
                $store_id = $this->request->data['DeliveryLocation']['store_id'];
                $delivery_exists = $this->DeliveryLocation->find('all', array(
                    'conditions' => array('DeliveryLocation.store_id' => $store_id,
                        'DeliveryLocation.city_id' => $this->request->data['DeliveryLocation']['city_id']
                    )
    ...
    //delivery edit
    public function store_deliveryDetail($id = null) {
        $this->layout = 'assets';
        $stores_id = $this->Auth->User();
        $store_id = $stores_id['Store']['id'];
        if (is_numeric($id)) {
            ...
            $checkLocation = $this->DeliveryLocation->find('first', array(
                'conditions' => array(
            ...
            if (!empty($checkLocation) && !empty($id)) {

                if ($this->request->is('post') || $this->request->is('put')) {
                    $this->DeliveryLocation->set($this->request->data);

                    $this->DeliveryLocation->save($this->request->data, null, null);
                    $this->Session->setFlash('<p>' . __('Informations de livraison mises à jour', true) . '</p>', 'default', array('class' => 'alert alert-success'));
                    $this->redirect(array('controller' => 'Orders', 'action' => 'displayDelivery'));
...
//                    }
                    ...
                    $getStateData = $this->DeliveryLocation->findById($id);
                ...
                $this->Session->setFlash('<p>' . __("L'information de livraison n'est pas valide", true) . '</p>', 'default', array('class' => 'alert alert-danger'));
                $this->redirect(array('controller' => 'Orders', 'action' => 'displayDelivery'));
            }
        } else {
            $this->Session->setFlash('<p>' . __('Vous avez mis à jour le formulaire', true) . '</p>', 'default', array('class' => 'alert alert-danger'));
            $this->redirect(array('controller' => 'Orders', 'action' => 'displayDelivery'));
        }
    }

    public function admin_deliveryDetail($id = null) {
        $this->layout = 'assets';
...
//                    $delivery_exists = $this->DeliveryLocation->find('all', array(
//                        'conditions' => array('DeliveryLocation.store_id' => $checkLocation['Store']['id'],
//                            'DeliveryLocation.city_id' => $this->request->data['DeliveryLocation']['city_id']
//                        )
//                            )
//                    );
...
//                    if (!empty($delivery_exists)) {
//                        $this->Session->setFlash('<p>' . __('Les informations de livraison existent déjà', true) . '</p>', 'default', array('class' => 'alert alert-danger'));
//                        $this->redirect(array('controller' => 'Orders', 'action' => 'displayDelivery',$checkLocation['Store']['id']));
//                    } else {
//
//                        $this->DeliveryLocation->save($this->request->data, null, null);
//                        $this->Session->setFlash('<p>' . __('Informations de livraison mises à jour', true) . '</p>', 'default', array('class' => 'alert alert-success'));
...
//                    }
                    ...
                    $getStateData = $this->DeliveryLocation->findById($id);
                    $this->request->data = $getStateData;
                    $this->set('store_id', $checkLocation['Store']['id']);
                    $this->set('delivery_id', $id);
                }
            } else {
                $this->Session->setFlash('<p>' . __("L'information de livraison n'est pas valide", true) . '</p>', 'default', array('class' => 'alert alert-danger'));
    ...
    public function admin_displayDelivery($id = null) {

        $deliveryAreas = array();
        if ($id != '') {
            $deliveryAreas = $this->DeliveryLocation->find('all', array(
                'conditions' => array(
                    'DeliveryLocation.store_id' => $id
                )
                    )
            );
        } else {
        ...
        $this->set(compact('deliveryAreas', 'stores'));
    }
    ...
    public function store_orderStatus() {
        ...
        if ($orderStatusUpdate['status'] == 'Accepted_Asap') {
            $orderStatusUpdate['status'] = "Accepted";
            $orderStatusUpdate['driver_id'] = '';
            $orderStatusUpdate['estimated_pickup_time'] = $this->request->data['estimated_pickup_time'];
            ...
            $orderStatusUpdate['status'] = "Accepted_Asap";
        }
        ...
        if ($_SERVER['HTTP_HOST'] == 'halal-resto.fr') {
            ...
            if ($orderStatusUpdate['status'] == 'Accepted_Asap' || $orderStatusUpdate['status'] == 'Accepted' || $orderStatusUpdate['status'] == 'Delivered' || ($orderStatusUpdate['status'] == 'Failed')) {
    ...
    public function store_collectionOrder() {
        ...
        /* $pendingStatus = array('Pending' => 'En attente', 'Accepted' => 'Accepté', 'Failed' => 'Annulé');
          $acceptStatus  = array('Pending' => 'Accepté', 'Failed' => 'Annulé', 'Delivered' => 'Livré'); */
        $status = array('Pending' => 'En attente', 'Accepted' => 'Accepté',
            'Failed' => 'Annulé', 'Delivered' => 'Expédié');
        $this->set(compact('order_list', 'status', 'order_lists'));
    }
    ...
    public function admin_refund($orderId, $failed = null) {
        ...
        if (!empty($orderDetail)) {
            ...
            if (!empty($orderDetail['Order']['transaction_id'])) {
                ...
                } else if ($orderDetail['Order']['payment_type'] == 'paypal' &&
                        $orderDetail['Order']['payment_method'] == 'paid' &&
                        $orderDetail['Order']['status'] != 'Delivered') {
    ...
    public function paymentsuccess() {
        ...
        if (!empty($this->Session->read('paypalOrders'))) {
            ...
            $transactionid = $this->request->query['PayerID'];
            $paymentarray = $this->Paypal->GetShippingDetails($token);

            if ($paymentarray['PAYERSTATUS'] == "verified" && $paymentarray['ACK'] == "Success") {

                $this->Order->save($this->Session->read('paypalOrders'), null, null);
                ...
                $this->changeLocation();
                $this->Session->write("preSessionid", '');
                $this->Session->write('paypalOrders', '');
                $this->Session->setFlash(__('Your Order Placed Successfully', true), 'default', array('class' => 'alert alert-success'));
                $this->redirect(array('controller' => 'checkouts', 'action' => 'thanks',
                    $this->Order->id));
            }
