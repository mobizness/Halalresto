...
class CheckoutsController extends AppController {
	...
	public function index() {
        ...
        $this->set(compact('openCloseStatus', 'firstStatus', 'secondStatus', 'customerDetails','shopCart',
        			'taxDetails','offerDetails','addresslist','addresses', 'storeDetails', 'stripeCards'));
        ...
        if ($this->siteSetting['Sitesetting']['address_mode'] != 'Google') {
			...
			if ($orderTypes == 'Delivery') {
				...
				if ($deliveryDistance < $Duration) {
					...
					echo $this->request->data['total'] + $shopCartDetails['Store']['delivery_charge'];
				}
			}
        }
		exit();
