...
class CustomersController extends AppController {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if ($this->Customer->validates()) {
                ...
                if (!empty($CustomerExist) || !empty($StoreExists)) {
                    ...
                    $this->Session->setFlash('<p>' . __('Utilisateurs déjà existant', true) . '</p>', 'default', array('class' => 'alert alert-danger'));
                } else {
                    ...
                    $this->Session->setFlash('<p>' . __('Les utilisateurs ont été enregistré', true) . '</p>', 'default', array('class' => 'alert alert-success'));
                    $this->redirect(array('controller' => 'Customers', 'action' => 'index'));
                }
            } else {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if ($this->Customer->validates()) {
                ...
                if (!empty($customerEmailCheck) || !empty($StoreExists)) {
                    ...
                    $this->Session->setFlash('<p>' . __('Votre client a été enregistré', true) . '</p>', 'default', array('class' => 'alert alert-success'));
                    $this->redirect(array('controller' => 'Customers', 'action' => 'index'));
                }
            } else {
                $this->Customer->validationErrors;
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if ($this->CustomerAddressBook->validates()) {
                ...
                if (!empty($address_check)) {
                    $this->Session->setFlash('<p>' . __('Le carnet d’adresses existe déjà', true) . '</p>', 'default', array('class' => 'alert alert-danger'));
                } else {
                    $this->CustomerAddressBook->save($this->request->data, null, null);
                    $this->Session->setFlash('<p>' . __('Votre carnet d’adresses client a été enregistré', true) . '</p>', 'default', array('class' => 'alert alert-success'));
                    $this->redirect(array('controller' => 'Customers', 'action' => 'index'));
                }
            } else {
    ...
    public function customer_myaccount() {
        ...
        if (!empty($this->request->data['Customer']['customer_phone']) && !empty($this->request->data['Customer']['first_name'])) {
            ...
            if ($this->Auth->User('role_id') == 4) {
                ...
                $this->Customer->save($this->request->data, null, null);
                $this->Auth->login();
                $this->Session->setFlash('<p>' . __('Vos informations ont été enregistrées', true) . '</p>', 'default', array('class' => 'alert alert-success'));
        ...
        if (!empty($this->request->data['review']['rating'])) {
            ...
            if (empty($checking)) {
                ...
                $this->Session->setFlash('<p>' . __('Thank you for your review', true) . '</p>', 'default', array('class' => 'alert alert-success'));
            } else {
                $this->Session->setFlash('<p>' . __('Review Already Exsits', true) . '</p>', 'default', array('class' => 'alert alert-danger'));
                $this->redirect(array('controller' => 'Customers', 'action' => 'myaccount'));
            }
        }
        $ids = $this->Auth->User();
        ...
        $order_detail = $this->Order->find('all', array(
            'conditions' => array(
                ...
                'OR' => array(
                    ...
                    'Order.customer_id' => 'Collected',
                )
            ),
            'order' => array('Order.id DESC')));

        //Pending Or Accepted Orders
        $follow_up_orders = $this->Order->find('all', array(
            'conditions' => array(
                'Order.customer_id' => $ids['Customer']['id'],
                'NOT' => array(
                    'Order.status' => array(
        ...
        $this->set(compact('state_list', 'addressBook','follow_up_orders', 'order_detail', 'Stripe_detail', 'month', 'walletHistory', 'storeId'));
    }
    ...
    public function customer_customerCardAdd() {
        ...
        $this->Session->setFlash('<p>' . __('Votre carte a été ajoutée avec succès', true) . '</p>', 'default', array('class' => 'alert alert-success'));
        $this->redirect(array('controller' => 'customers', 'action' => 'myaccount'));
    }
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if ($this->CustomerAddressBook->validates()) {
                ...
                if (!empty($customerAddressBook)) {
                    $this->CustomerAddressBook->save($this->request->data, null, null);
                    $this->Session->setFlash('<p>' . __('Your address book has been updated successfully', true) . '</p>', 'default', array('class' => 'alert alert-success'));
                    $this->redirect(array('controller' => 'customers', 'action' => 'myaccount'));
                }
            } else {
                $this->CustomerAddressBook->validationErrors;
    ...
    public function customer_pdf($id = null) {
        ...
        if (empty($order_detail)) {
            ...
            $order_detail['Order']['delivery_time'] = ($order_detail['Order']['assoonas'] == 'Later') ? $order_detail['Order']['delivery_time'] : 'ASAP';
    ...
    public function customer_changePassword() {
        ...
        if (trim($check_password) == trim($old_password)) {
            if ($newpassword == $confirmpassword && $newpassword != '' && $confirmpassword != '') {
                ...
                $this->Session->setFlash('<p>' . __('Vérifier votre nouveau de passe et confirmez le', true) . '</p>', 'default', array('class' => 'alert alert-danger'));
                $this->redirect(array('controller' => 'Customers', 'action' => 'myaccount'));
            }
        } else {
            $this->Session->setFlash('<p>' . __('Vérifier votre ancien mot de passe', true) . '</p>', 'default', array('class' => 'alert alert-danger'));
            $this->redirect(array('controller' => 'Customers', 'action' => 'myaccount'));
        }
    }
        ...
        if (!empty($this->request->data)) {
            ...
            if (!empty($stripeCard)) {
                ...
                if ($stripeResponse['status'] == "succeeded" && $stripeResponse['stripe_id'] != '') {
                    ...
                    $this->Session->setFlash('<p>' . __('Le montant a été ajouté avec succès dans votre portefeuille', true) . '</p>', 'default', array('class' => 'alert alert-success'));
                } else {
                    $this->Session->setFlash('<p>' . __('Le paiement a échoué', true) . '</p>', 'default', array('class' => 'alert alert-danger'));
                }
            } else {
                $this->Session->setFlash('<p>' . __('La carte n’est pas valide', true) . '</p>', 'default', array('class' => 'alert alert-danger'));
            }
        }
        $this->redirect(array('controller' => 'customers', 'action' => 'myaccount'));
    }
