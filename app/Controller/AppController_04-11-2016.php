...
class AppController extends Controller {
    ...
    public function isAuthorized($user) {
        ...
        if (($this->params['prefix'] === 'store') && ($user['role_id'] != 2 && $user['role_id'] != 3)) {
           echo '<h3 class="form-title"> Vous n’êtes pas autorisé à accéder à cette page </h3>
                    <a href="'.$this->siteUrl.'/users/logout/store"> Cliquez ici pour vous déconnecter  </a>';
            exit();
        }
	...
	public function basicSetup() {
        ...
        $this->siteUrl = 'http://'.$_SERVER['SERVER_NAME'].'/halal';
        // $this->siteUrl = 'http://'.$_SERVER['SERVER_NAME'].'/dev';
		$this->set('siteUrl', $this->siteUrl);

		$this->siteName = 'http://'.$_SERVER['SERVER_NAME'].'/halal';
        // $this->siteName = 'http://'.$_SERVER['SERVER_NAME'].'/dev';
        ...
        $liveUrl = array('www.foodorderingsystem.com', 'foodorderingsystem.com');
