...
class UsersController extends AppController {
	...
	public $uses = array('User', 'Customer','Notification', 'Store', 'ContactUs', 'StoreTiming','City');
	...
	public function beforeFilter() {
		...
		$this->Auth->allow(array('signup', 'customer_customerLogin','storeLogin','storeActiveLink',
     ...
     */  
	 ...
	 public function store_storeLogin() {
		if ($this->Auth->loggedIn() && $this->Auth->user('role_id') == 3) {
			...
			echo '<h3 class="form-title"> You are not authorized to access this Page </h3>
        			<a href="'.$this->siteUrl.'/users/logout/restaurant"> Click here to Logout  </a>';
			exit();      	
		...
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			if(isset($this->request->data['Users']['email']) && $this->request->data['Users']['email'] != ''){
				   ...
				   $datas['User']['id'] = $userData['User']['id'];
					   ...
					   if($email->send()){
						// Forget Sms
					   	$storeMessage = "We've received a request to change your password. Use this password ".$tmpPassword." to login to your account and update it ASAP. Thanks '.$this->siteSetting['Sitesetting']['site_name'].'";
			...
			if($this->User->validates()) {
				...
				if(in_array($userData['User']['role_id'],$roles)) {
                    if ($userData['Store']['status'] == 1) {
                        ...
                        $this ->Session->setFlash('<p>'.__('Your account is in pending', true).'</p>', 'default',
                            array('class' => 'alert alert-danger'));
                        $this->redirect(array('controller' => 'users', 'action' => 'storeLogin','store'=>true));
                    }
				} else {
    ...
    */
	...
	public function storeSignup() {
		...
		if ($this->Auth->loggedIn()) {
			echo '<h3 class="form-title"> You are not authorized to access this Page </h3>
        			<a href="'.$this->siteUrl.'/users/logout/storeSignup"> Click here to Logout  </a>';
			exit();
		}
                    ...
                    if ($this->User->save($storeArrray, null, null)) {
                        ...
                        if($this->Store->save($storeArrray, null, null)) {
                            ...
                            $email->viewVars(array('mailContent' => $mailContent,
                            						...
                            						'storename' => $siteName));
                            $email->send();
                            $this->Session->setFlash('<p>'.__('Restaurant details stored successfully', true).'</p>', 'default',
                                array('class' => 'alert alert-success'));
    ...
    public function storeActiveLink($id = null){
        ...
        if ($id) {
            ...
            if (!empty($storeDetails)) {
                ...
                if ($this->Store->updateAll(array('Store.status' => 1), array('Store.id' => $id))) {
                    $this->Session->setFlash('<p>'.__('Your account is activated', true).'</p>', 'default',
                        array('class' => 'alert alert-success'));
                    $this->redirect(array('controller' => 'users', 'action' => 'storeLogin', 'store' => true));
                }
            } else {
                $this->Session->setFlash('<p>'.__('You are not registered Restaurant.', true).'</p>', 'default',
                    array('class' => 'alert alert-danger'));
                $this->redirect(array('controller' => 'users', 'action' => 'storeSignup'));
            }
        }
       		...
       		if($this->Customer->validates()) {  
				...
				$emailinfo .='<table>
							<tr style="display:block; width:610px; margin-bottom:15px;font:13px Arial, Helvetica, sans-serif;">
		    					<td colspan="3" style="display:inline-block; width:100%;vertical-align: top;">Dear Admin,</td>		    					
		    				</tr>
		    				<tr style="display:block; width:610px; margin-bottom:15px;font:13px Arial, Helvetica, sans-serif;">
		    					<td colspan="3" style="display:inline-block; width:100%;vertical-align: top;">Please review a new apply Devenez coursier, postulez application form from the Devenez coursier, postulez. Please check the following details</td>		    					
		    				</tr>	
							<tr style="display:block; width:610px; margin-bottom:15px;font:13px Arial, Helvetica, sans-serif;">
		    					<td style="display:inline-block; width:185px;vertical-align: top;">Name</td>
		    					<td style="display:inline-block; width:50px; text-align:center;">:</td>
		    					<td style="display:inline-block; width:350px; color:#ee541e;">'.$cusFirstName.' '.$cusLastName.'</td>
		    				</tr>
		                    <tr style="display:block; width:610px; margin-bottom:15px;font:13px Arial, Helvetica, sans-serif;">
		                    ...
		                    <tr style="display:block; width:610px; margin-bottom:15px;font:13px Arial, Helvetica, sans-serif;">
		    					<td style="display:inline-block; width:185px;vertical-align: top;">Phone</td>
		    					<td style="display:inline-block; width:50px; text-align:center;">:</td>
		    					<td style="display:inline-block; width:350px; color:#ee541e;">'.$custPhone.'</td>
		    				</tr>
		                    <tr style="display:block; width:610px; margin-bottom:15px;font:13px Arial, Helvetica, sans-serif;">
		    					<td style="display:inline-block; width:185px;vertical-align: top;">City</td>
		    					<td style="display:inline-block; width:50px; text-align:center;">:</td>
		        ...
		        $email->send();
             	...
             	$this->Session->setFlash('<p>'.__('You have successfully appplied', true).'</p>', 'default', 
                                                  array('class' => 'alert alert-success'));
