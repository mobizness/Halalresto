...
class OrdersController extends AppController {
  ...
  public $uses = array('Order','ShoppingCart', 'CustomerAddressBook', 'StripeCustomer',
                          ...
                          'ProductDetail', 'Notification', 'StripeRefund', 'Voucher',
                          'DeliveryLocation', 'Customer', 'WalletHistory');

  public $components = array('Stripe', 'Googlemap', 'AndroidResponse', 'Twilio', 'Notifications');
  ...
  public function admin_index() {
  	...
  	$order_list = $this->Order->find('all', array(
                        'conditions' => array('Order.status' => array('Pending', 'Accepted')),
                        'order' => array('Order.id DESC')));
    $pendingStatus = array('Pending' => 'Pending', 'Accepted' => 'Accepted', 'Failed' => 'Failed');
    $acceptStatus  = array('Pending' => 'Accepted', 'Failed' => 'Failed', 'Delivered' => 'Delivered');
    $this->set(compact('order_list', 'pendingStatus', 'acceptStatus'));
  }
  ...
  public function admin_reportIndex($storeId = null, $rangeId = null) {
    
      list($order_list) = $this->orderFilterReport($storeId, $rangeId);

      $range = array('0' => 'All', '1' => 'Today', '2' => 'This Week',
                      '3' => 'This Month', '4' => 'This Year');
      ...
      $this->set(compact('order_list', 'stores', 'range'));     
  }
  ...
  public function conformOrder() {
      ...
      if ($this->request->data['Order']['paymentMethod'] == 'cod') {
        ...
        $amount     = $order['order_grand_total'];
        ...
        if ($this->request->data['Order']['paymentMethod'] == 'wallet') {
          ...
          if ($stripeResponse['status'] == "succeeded" && $stripeResponse['stripe_id'] != '') {
            ...
            $this->Session->setFlash(__('Payment failed', true),
                                      'default', array('class' => 'alert alert-danger'));
            $this->redirect(array('controller' => 'customers', 'action' => 'orderView', 
                                    $this->Order->id, 'customer' => true));
          }
        }
  ...
  public function ordersms($orderId) {
      ...
      $customerMessage = 'Thanks for using '.$this->siteSetting['Sitesetting']['site_name'].' service. Your order '.$orderDetail['Order']['ref_number'].' has been placed .';
  ...
  public function store_orderIndex() {
    ...
    $order_list   = $this->Order->find('all', array(
                            'conditions'=>array('Order.store_id'=>$this->Auth->User('Store.id'),
                                                'Order.status' => array('Pending', 'Accepted')),
                             'order' => array('Order.id DESC')));
    $pendingStatus = array('Pending' => 'Pending', 'Accepted' => 'Accepted', 'Failed' => 'Failed');
    $acceptStatus  = array('Pending' => 'Accepted', 'Failed' => 'Failed', 'Delivered' => 'Delivered');
    $this->set(compact('order_list', 'pendingStatus', 'acceptStatus'));
  ...
  public function admin_orderStatus() {
    ...
    if ($orderStatusUpdate['status'] == 'Accepted') {
        if ($this->Order->save($orderStatusUpdate)) {
            ...
            $customerMessage = 'Congratulations! Your order '.$orderDetail['Order']['ref_number'].'succesfully accepted by '.
                                ...
                                $orderDetail['Order']['delivery_date']. ' at '.$orderDetail['Order']['delivery_time'].'. Thanks '.$this->siteSetting['Sitesetting']['site_name'];
    ...
    $message = $orderDetail['Order']['ref_number'].' has been '.$orderStatusUpdate['status'];
  ...
  public function store_orderStatus() {
    ...
    if ($orderStatusUpdate['status'] == 'Accepted') {
        if ($this->Order->save($orderStatusUpdate)) {
            $customerMessage = 'Congratulations! Your order '.$orderDetail['Order']['ref_number'].' succesfully accepted by '.$orderDetail['Store']['store_name'].'. Your order will be delivered by '.$orderDetail['Order']['delivery_date']. ' at '.$orderDetail['Order']['delivery_time'].'. Thanks '.$this->siteSetting['Sitesetting']['site_name'];
  ...
  public function admin_refundNotification($orderId) {
    ...
    if (!empty($orderDetail)) {
      // Store Notification
      if (!empty($orderDetail['Store']['device_id']) && $orderDetail['Store']['is_logged'] == 1) {
  ...
  public function orderFilterReport($storeId = null, $rangeId = null) {
