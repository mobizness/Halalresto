...
class SearchesController extends AppController {
    ...
    public $uses = array('City', 'Location', 'Newsletters_user', 'Meat_issuer', 'Store', 'Product', 'Category', 'ProductDetail',
        'ShoppingCart', 'Storeoffer', 'Deal', 'DeliveryLocation', 'Review',
        'State', 'Cuisine', 'StoreCuisine', 'Subaddon', 'ProductAddon', 'Customer');
    public $components = array('Updown', 'Googlemap', 'Mailchimp');
    ...
    public function beforeFilter() {
        ...
        $this->name = $this->Session->read('Search.name');
    ...
    public function newslettersubscribe() {
        if (!empty($this->request->data["email"])) {
            ...
            $count = $this->Newsletters_user->find("all", array(
                'conditions' => array(
                    'email' => $email
                )
            ));
            if (sizeof($count) == 0) {
                $this->Store->query("INSERT INTO newsletters_users(email) VALUES ('$email')");
                $merge_vars = array(
                    'EMAIL' => $email,
                    'FNAME' => $email,
                    'LNAME' => $email
                ...
                $list = $this->Mailchimp->listSubscribe($this->mailChimpListId, $email, $merge_vars);
...
//        }
        ...
        if ($this->siteSetting['Sitesetting']['address_mode'] != 'Google') {
            ...
            if (!empty($this->request->data['Search']['name'])) {
                $searchName = $this->request->data['Search']['name'];
                $this->Session->write('Search.name', $searchName);
                $this->redirect($this->siteUrl . '/address');
            }else{
                 $this->Session->write('Search.name', "");
            }
        }

//Store offers in home page
        $today = date("m/d/Y");
        $sun = date('m/d/Y', strtotime('Next Sunday', strtotime($today)));

        $prodLists = $this->Product->find('all', array(
            'conditions' => array(
                'Store.status' => 1,
                'Product.status' => 1,
...
// echo "---------<pre>";print_r($prodLists);echo "</pre>";exit;

        /* foreach ($prodLists as $key => $value) {
...
// echo "---------<pre>";print_r($storeList);echo "</pre>";exit;

        $storeOffers = [];
        $i = 0;
        foreach ($prodLists as $key => $val) {


            $storeOffer = $this->Storeoffer->find('first', array(
                'conditions' => array('Storeoffer.status' => 1,
                    'Storeoffer.store_id' => $val['Store']['id'],
                    'AND' => array("Storeoffer.from_date >=" => $today, "Storeoffer.from_date <=" => $sun),
                    'OR' => array("Storeoffer.to_date <=" => $sun, "Storeoffer.to_date >=" => $sun)),
                'order' => 'Storeoffer.id DESC'));


            /* $productList = $this->Product->find('first', array(
              'conditions' => array('Product.store_id' => $val,
              'Product.status' => 1,
              'MainCategory.status' => 1,
              'OR' => array('Store.collection' => 'Yes',
              'Store.delivery'	 => 'Yes')),
            ...
            $rating = (!empty($ratingDetail[0]['ratingCount'])) ?
                    ($ratingDetail[0]['rating'] / $ratingDetail[0]['ratingCount']) * 20 : 0;

            if (!empty($storeOffer) && $i <= 5) {
                $storeOffers[$i] = $storeOffer;
                $storeOffers[$i]['Store']['rating'] = $rating;
                $storeOffers[$i]['Store']['product_image'] = $prodLists[$key]['Product']['product_image'];
                $i++;
            }
        }

// echo "---------<pre>";print_r($storeOffers);echo "</pre>";exit;
...
//Featured Restaurants
        ...
        $featured_stores = $this->Store->find('all', array(
            'conditions' => array(
                'Store.status' => 1,
                'Store.isFeatured' => 1,
                'Store.featured_start_time <=' => $currentDate,
                'Store.featured_end_time >=' => $currentDate
            ),
            'group' => array(
                'Store.id'
            ),
        ...
        $featuredArray = array();
        if (sizeof($featured_stores) > 6) {
            $featuredArray = $this->UniqueRandomNumbersWithinRange(0, sizeof($featured_stores) - 1, 6);
        }

        $homepage_youtube_link = !empty($this->siteSetting['Sitesetting']['homepage_youtube_link']) ? $this->siteSetting['Sitesetting']['homepage_youtube_link'] : "https://www.youtube.com/embed/kJQz3eFFm6I";
        $this->set(compact('cityList', 'homepage_youtube_link', 'storeOffers', 'featured_stores', 'featuredArray'));
    }
...
// Pre Order or Order
            ...
            if ($openCloseStatus == 'Open') {
                $value['Store']['status'] = ($firstStatus == 'Open' || $secondStatus == 'Open') ? 'Order' : 'Pre Order';
            } else {
        ...
        if (empty($storeList)) {
            $this->Session->setFlash('<p>' . __('Le restaurant n’est pas disponible', true) . '</p>', 'default', array('class' => 'alert alert-danger'));
            $this->redirect(array('controller' => 'searches', 'action' => 'index'));
        }
    ...
    public function searchByAddress() {
        ...
        if (empty($this->address) && empty($this->name)) {
            $this->redirect(array('controller' => 'searches', 'action' => 'index'));
        }
        ...
        if (empty($this->address) && !empty($this->name)) {
            ...
            $storeLists = $this->Product->query("SELECT Store.id as id FROM products as p LEFT JOIN"
                    ...
                    . "(Store.store_name LIKE '%$serachAddress%')"
...
//if ($deliveryDistance >= $Duration) {
        ...
        $this->set(compact('getStoreCuisine', 'serachAddress', 'storeList', 'cuisineName', 'meat_issuer_name'));
        $this->render('stores');
    }
    ...
    public function storeitems($storename, $storeId) {
        ...
        if (!empty($storeDetails) && $this->siteSetting['Sitesetting']['address_mode'] == 'Google') {
            ...
            $Duration = $this->Googlemap->getDistance(
                    $this->address, $storeDetails['Store']['address']
            );
        ...
        foreach ($storeDetails['Review'] as $key => $value) {
            if ($value['status'] == 1) {
                ...
                $customer = $this->Customer->findById($value['customer_id']);
                $storeDetails['Review'][$key]['customer_name'] = $customer['Customer']['first_name'] . '' . $customer['Customer']['last_name'];
            } else {
                $storeDetails['Review'][$key] = '';
            }
        }

        $this->Session->write('storeId', $storeId);

        $this->set(compact('storeList', 'productList', 'storeDetails', 'mainCategoryList', 'storeId', 'dealProduct', 'metaTitle', 'metakeywords', 'metaDescriptions', 'storeOffers', 'dealProducts', 'guests', 'times', 'ratingDetail'));
    }
